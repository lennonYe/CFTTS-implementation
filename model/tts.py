...
from model.cpc import CPCCodeNet
...
def compute_cpc_loss(cpc_code, cpc_anchor, neg_samples=10):
    """
    Compute CPC loss

    参数:
    - cpc_code: embedding of positive samples. (batch_size, length, code_dim)。
    - cpc_anchor: embedding of future representations. (batch_size, length, code_dim)。
    - neg_samples: Numbers of neative samples for each time step
    ...
    # Get dimensions
    batch_size, length, code_dim = cpc_code.shape  # (batch_size, length, code_dim)

    # Calculate logits of positive examples
    pos_logits = torch.sum(cpc_code * cpc_anchor, dim=-1)  # (batch_size, length)

    # Generate negative samples index
    neg_indices = torch.randint(0, length, (batch_size, neg_samples, length), device=cpc_code.device)

    # Adjust shape to match cpc_anchor
    neg_indices = neg_indices.unsqueeze(-1).expand(-1, -1, -1, code_dim)  # (batch_size, neg_samples, length, code_dim)

    neg_samples_gathered = torch.gather(cpc_anchor.unsqueeze(1).expand(-1, neg_samples, -1, -1), 2, neg_indices)  # (batch_size, neg_samples, length, code_dim)

    neg_samples_gathered = neg_samples_gathered.permute(0, 2, 1, 3)  # (batch_size, length, neg_samples, code_dim)

    neg_logits = torch.einsum('bld,blnd->bln', cpc_code, neg_samples_gathered)  # (batch_size, neg_samples, length)

    logits = torch.cat([pos_logits.unsqueeze(2), neg_logits], dim=2)  # (batch_size, seq_len, num_classes)

    labels = torch.zeros(batch_size, length, dtype=torch.long, device=cpc_code.device)  # (batch_size, seq_len)

    logits = logits.view(-1, logits.size(-1))  # (N * T, C)
...
def generate_reverse_idx_for_batch(y_lengths):
    batch_size = y_lengths.size(0)
    max_seq_len = y_lengths.max().item()
    reverse_indices = torch.zeros((batch_size, max_seq_len), dtype=torch.long, device=y_lengths.device)
    
    for i, seq_len in enumerate(y_lengths):
        reverse_idx = torch.arange(seq_len - 1, -1, -1, device=y_lengths.device).long()
        reverse_indices[i, :seq_len] = reverse_idx
...
class GradTTS(BaseModule):
    def __init__(self, n_vocab=148, n_spks=1, spk_emb_dim=64,
        ...
        # self.hubert_model = HubertModel.from_pretrained("facebook/hubert-large-ll60k")
        self.cpc = CPCCodeNet()
        ...
        self.dec_lowrank = torch.nn.Linear(144,80)
        self.hubert_proj = torch.nn.Linear(1024, 80)
        for param in self.hubert_proj.parameters():
    ...
    def inference(self, x, x_lengths, n_timesteps, temperature=1.0, spk=None, length_scale=1.0, solver="dopri5", gt_dur=None):
        ...
        cpc_inference = self.cpc.extract(mu_y.permute(0,2,1))
        dec_input = torch.cat((mu_y.permute(0,2,1),cpc_inference),dim=2)
        dec_input = self.dec_lowrank(dec_input)
        dec_input = dec_input.permute(0,2,1)


        decoder_output_traj = self.decoder.inference(z, y_mask, dec_input, n_timesteps, spk, solver=solver)
    ...
    def forward(self, x, x_lengths, y, y_lengths, noise, spk=None, out_size=None, use_gt_dur=False, durs=None,hubert_embedding=None):
        """
        Computes 3 losses:
        ...
        hubert_embedding_proj = self.hubert_proj(hubert_embedding)
        hubert_seq_len = hubert_embedding_proj.size(1)
        mu_x_seq_len = mu_x.size(2)
        
        if hubert_seq_len != mu_x_seq_len:
            hubert_embedding_proj = F.interpolate(
                hubert_embedding_proj.permute(0, 2, 1),  #[batch_size, 80, hubert_seq_len]
                size=mu_x_seq_len,  
                mode='linear',  
                align_corners=False
            )
        mu_x_mask = (mu_x != 0).float()
        hubert_loss = F.mse_loss(hubert_embedding_proj * mu_x_mask, mu_x * mu_x_mask)
        ...
        # Cpcloss added
        reverse_idx = generate_reverse_idx_for_batch(y_cut_lengths).to('cuda')

        CPCCode, CPCAnchor = self.cpc(mu_y.permute(0,2,1).to('cuda'),reverse_idx)
        

        # import pdb;pdb.set_trace()
        cpc_loss = compute_cpc_loss(CPCCode, CPCAnchor)

        dec_input = torch.cat((mu_y.permute(0,2,1),CPCCode),dim=2)
        dec_input = self.dec_lowrank(dec_input)
        dec_input = dec_input.permute(0,2,1)
        # Compute loss of score-based decoder
        # diff_loss, xt = self.decoder.forward(y, noise, y_mask, mu_y, spk)
        diff_loss, xt = self.decoder.forward(y, noise, y_mask, dec_input, spk)
        ...
        # Compute cpc loss
        return dur_loss, prior_loss, diff_loss, l_mle, cpc_loss, hubert_loss
