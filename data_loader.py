...
class BaseLoader(torch.utils.data.Dataset):
    def __init__(self, utts: str, n_mel_channels: int, sampling_rate: int,
                 ...
                 utt2phn_duration: str, add_blank=False, noise_scp: str = None, utt2wav: str = None):
        """
        ...
        if utt2wav is not None:
            self.utt2wav = self.get_utt2wav(utt2wav)
        else:
            self.utt2wav = None
    ...
    def sample_test_batch(self, size):
     ...
     # Method to read wav.scp file
    def get_utt2wav(self, utt2wav: str) -> dict:
        res = dict()
        with open(utt2wav, 'r') as f:
            for l in f.readlines():
                uttid, wavpath = l.strip().split()
                res[uttid] = wavpath
        return res

    # Method to get wav path
    def get_wav_path(self, utt):
        if self.utt2wav:
            return self.utt2wav[utt]
        else:
            return None
            # raise ValueError("utt2wav is not set for this loader.")


class SpkIDLoader(BaseLoader):
    def __init__(self, utts: str, n_mel_channels: int, sampling_rate: int,
                 feats_scp: str, utt2num_frames:str, utt2phns: str, phn2id: str,
                 utt2phn_duration: str, utt2spk: str, add_blank: bool = False,  noise_scp: str = None,utt2wav: str = None):
        """
        ...
        super(SpkIDLoader, self).__init__(utts, n_mel_channels, sampling_rate, feats_scp, utt2num_frames, utt2phns, phn2id, utt2phn_duration,
                                          add_blank=add_blank, noise_scp=noise_scp,utt2wav=utt2wav)
    ...
    def get_mel_text_pair(self, utt):
        ...
        wav_path = self.get_wav_path(utt)  # Get wav file path
...
class XvectorLoader(BaseLoader):
    def __init__(self, utts: str, n_mel_channels: int, sampling_rate: int,
                 feats_scp: str, utt2num_frames: str, utt2phns: str, phn2id: str,
                 utt2phn_duration: str, utt2spk_name: str, spk_xvector_scp: str, add_blank: bool = False, noise_scp: str = None, utt2wav: str = None):
        """
